...
This document contains an example of a model setup primarily for fitting and model comparison.
It is setup in a way that helps you learn the method step by step. We recommend working through this file step by step and working to understand what it is doing. Then work through the model 2 rmd file. After that, we recommend making a copy of the model 2 file and modifying the model to make your own model of the disease. The model 2 file is more streamlined for modifying to implement your own model once you understand what all of these pieces do. This method hopefully will help you learn the method and learn how to troubleshoot problems should you encounter them.   
...
Some of these chunks of code take a very long time to run. This can make knitting this file take a very long time. You make just want to play with the individual chunks and not knit the file each time. 
...
knitr::opts_chunk$set(
                    ...
                    warning=TRUE,
                    include=TRUE,
                    #cache=TRUE, #this and the next line can help speed up knitting
                    #cache.path=paste0("temp/","SI/","cache/"),
                    progress=TRUE
                  )
```
...
First, lets make sure that your working directory is set to be the same as where this rmd file is stored. This makes sure that additional files end up in the same place.  
```{r savefile}
library(rstudioapi)
...
Now we take our model and use it to set up the model for each step. When troubleshooting it can be useful to setup the function in R but then for speed switching to Csnippets is helpful. So we show both here but use the Csnippet version for the rest of this code.
...
Setup the initial conditions. These are defined in a way that allows us to change the parameter defining the fraction of the population that is infected ($fI$) if we want to. The initial conditions need to be integers so the ```nearbyint()``` function helps ensure that the are integers. 
```{r init_R}
si_rinit <- function(N, fI, ...){
  c(S = round((1-fI)*N), 
    I= round(fI*N), NewI=0)
}
```
...
#These two approaches should match but are not
ggplot()+
...
```{r pfilter 1}
pf <- SI_Model |>
  pfilter(params= c(N=1000, fI = 0.004,Beta=.0024, gamma=.1, rho=.8, k=5 ),
          Np=100)
logLik(pf)
```
...
foreach (i=1:10, .combine=c) %dopar% {
  ...
  SI_Model |>   
    pfilter(
      params= c(N=1000, fI = 0.004,Beta=.0024, gamma=.1, rho=.8, k=5 ),
      Np=100 #Later you might want Np=1000
      )
} -> pf
logLik(pf) -> ll
ll
...
mifs_local <-
  ...
  mif2(
    params= c(N=1000, fI = 0.004,Beta=.0024, gamma=.1, rho=.8, k=5 ),
    Np=100, #Set small for speed, later increase to Np=1000
...
Decide on a name for your model results. Our two models for this example will be the SI model and later the SIR model so ``SI_Model_results.csv'' will work for our model name and we can add the ``R'' when we get to the second model. 
```{r}
...
guesses <- 
  runif_design(
    lower=c(Beta=0.0001,gamma=0.05,rho=0.1,k=1,N=1000,fI=0.004),
    upper=c(Beta=0.05,gamma=0.5,rho=0.9,k=10,N=1000,fI=0.004),
    nseq=50 #change to nseq=200 later 
    #This is reduced for getting code working. 
  ) 
...
foreach(
    ...
    .combine=bind_rows
      ...
      mf <- mif2(
        ...
        Np=100, #number of particles set to Np=1000 later
        Nmif=25, #number of iterations set to Nmif=50 later
        cooling.fraction.50=0.5,  
        partrans=par_trans)
      
      #estimate the likelihood 10 times for each resulting parameter set
      ll <- mf |>
        pfilter(Np=100) |> #set to Np=1000 later
        logLik() |>
        replicate(n=10) |>
...
Add the results to the saved CSV file (or overwrites/makes a new file if MAKEFILE is set to TRUE above) and pulls the cumulative data from the saved csv file and names it "all". Then plot all of the data.
```{r}
append_data(data=results,file=savefile) -> all 
#You can overwrite saved results using the option ``overwrite=TRUE"

pairs(~loglik + Beta + gamma + k + rho ,data=all)
```

We can see that although the parameter sets started out evenly distributed across parameter space, the local search to maximize the likelihood have resulted in structured relationship between some pairs of variables. For example, it looks like the better combinations for ```rho``` and ```k``` have them linearly related to each other (with larger values of $k$ being associated with larger values of $\rho$). We also see that the best logliklihood values tend to be happening for small values of Beta. 
...
foreach(
    ...
    mifs_local |>
      mif2(
        ...
        Nmif=25, #Set Nmif=50 for final run
        rw.sd=rw_sd(Beta=0.02,gamma=0.02,rho=0.02,k=0.02)
        ) -> mf
      mf |>
        pfilter(Np=100) |>
...
Visualization of global result:
```{r}
pairs(~loglik + Beta + gamma + k + rho, data= all)
```

We can focus in on the area with higher likelihood values.
...
guesses <-
    profile_design(
      ...
      lower=c(box[1,c("Beta","rho","k")],N=1000,fI=0.004),
      upper=c(box[2,c("Beta","rho","k")],N=1000,fI=0.004),
      nprof=20,
      type="runif"
    )
...
gammaCI
```
If you don't see the likelihood drop below the confidence line on both sides, that is a sign that either we haven't searched sufficiently broadly to estimate the CI or the parameter is not well defined. 

Under this model, the infectious period is estimated to be $`r sort(signif(1/mc$ci[2],2))`$--$`r sort(signif(1/mc$ci[1],2))`$days. This seems like a pretty long time to have the infection. LEts simulate the results for one of the best parameter sets and see what it does.

```{r}
best_param <- all |>
  filter(loglik==max(loglik))|>
  select(N, fI, Beta,gamma,rho,k)
best_param
...
sims2 |>
  ...
  geom_line(aes(x=day,y=I,group=.id),color="gray")+
  guides(color="none")+
  theme_bw()
```
...
# Bonus: How many particles is enough?
You may want to skip this section and come back to it. It is helpful to learn the other parts first and then 
...
> I'm using `bake` and `freeze` in the following, to help speed up recompilation during development of the materials.
> It might be better from the pedagogical point of view to postpone this until later.
> Once the codes are pretty stable, one can simply remove those from the pipeline.

```{ pfilter Np dependence}
registerDoParallel()
...
foreach (Npart=particles) %do% {
  ...
  bind_rows() |>
  bake(
    file="pfilter-Np-dependence.rds",
    dependson=particles
  ) -> results
```
