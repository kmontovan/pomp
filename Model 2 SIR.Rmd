...
savefile <- "SIR_Model_results.csv"
...
guesses <- 
  runif_design(
    lower=c(Beta=0.0001,gamma=0.07,r=0.001, rho=0.1,k=1,N=1000,fI=0.004),
    upper=c(Beta=0.05,gamma=0.5,rho=0.9,r=0.05,k=10,N=1000,fI=0.004),
    nseq=400
  ) 
```
...
#add results to saved file and save all results saved so far to "all" for use here
...
guesses <-
    profile_design(
        ...
        lower=box[1,c("Beta","rho","r","k","N","fI")],
        upper=box[2,c("Beta","rho","r","k","N","fI")],
...
The number of particles chosen  affects the variability of the estimate. Here is one way you could look at the `est` and `se` across a range of values for `Np`.
